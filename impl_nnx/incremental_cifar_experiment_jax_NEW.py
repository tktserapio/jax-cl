...
class IncrementalCIFARExperiment(Experiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        # define jax device - handle GPU/CPU gracefully - just had to add this cuz i was running on my macbook at first
        try:
    ...
    def _initialize_summaries(self):
        ...
        train_prototype_array = jnp.zeros(total_checkpoints, dtype=jnp.float32)
        self.results_dict["train_loss_per_checkpoint"] = jnp.zeros_like(train_prototype_array)
        self.results_dict["train_accuracy_per_checkpoint"] = jnp.zeros_like(train_prototype_array)

        prototype_array = jnp.zeros(self.num_epochs, dtype=jnp.float32)
        self.results_dict["epoch_runtime"] = jnp.zeros_like(prototype_array)
        # test and validation summaries
        for set_type in ["test", "validation"]:
            self.results_dict[set_type + "_loss_per_epoch"] = jnp.zeros_like(prototype_array)
            self.results_dict[set_type + "_accuracy_per_epoch"] = jnp.zeros_like(prototype_array)
            self.results_dict[set_type + "_evaluation_runtime"] = jnp.zeros_like(prototype_array)
    ...
    def load_checkpoint_data_and_update_experiment_variables(self, file_path):
        ...
        for k, v in self.results_dict.items():
            self.results_dict[k] = partial_results[k] if not isinstance(partial_results[k], jnp.ndarray) else jnp.array(partial_results[k])
    ...
    def _store_training_summaries(self):
        # store train data - use JAX immutable updates
        self.results_dict["train_loss_per_checkpoint"] = self.results_dict["train_loss_per_checkpoint"].at[self.current_running_avg_step].add(self.running_loss / self.running_avg_window)
        self.results_dict["train_accuracy_per_checkpoint"] = self.results_dict["train_accuracy_per_checkpoint"].at[self.current_running_avg_step].add(self.running_accuracy / self.running_avg_window)
    ...
    def _store_test_summaries(self, test_data, val_data, epoch_number: int, epoch_runtime: float):
        ...
        self.results_dict["epoch_runtime"] = self.results_dict["epoch_runtime"].at[epoch_number].set(jnp.array(epoch_runtime, dtype=jnp.float32))
        ...
        for data_name, data_loader, compare_to_best in [("test", test_data, False), ("validation", val_data, True)]:
            ...
            # store summaries
            self.results_dict[data_name + "_evaluation_runtime"] = self.results_dict[data_name + "_evaluation_runtime"].at[epoch_number].set(jnp.array(evaluation_time, dtype=jnp.float32))
            self.results_dict[data_name + "_loss_per_epoch"] = self.results_dict[data_name + "_loss_per_epoch"].at[epoch_number].set(loss)
            self.results_dict[data_name + "_accuracy_per_epoch"] = self.results_dict[data_name + "_accuracy_per_epoch"].at[epoch_number].set(accuracy)
    ...
    def evaluate_network(self, test_data):
        ...
        for _, sample in enumerate(test_data):
            ...
            # Forward pass with static indices
            full_predictions = self.net(images)
            indices = self.possible_indices[self.current_num_classes]
            test_predictions = full_predictions[:, indices]
    ...
    def get_validation_and_train_indices(self, cifar_data: CifarDataSet):
        ...
        # Use JAX arrays instead of torch tensors
        validation_indices = jnp.zeros(validation_set_size, dtype=jnp.int32)
        train_indices = jnp.zeros(train_set_size, dtype=jnp.int32)
        current_val_samples = 0
        current_train_samples = 0
        for i in range(self.num_classes):
            # Use JAX/numpy operations instead of torch
            class_indices = jnp.where(cifar_data.data["labels"][:, i] == 1)[0]
            validation_indices = validation_indices.at[current_val_samples:(current_val_samples + num_val_samples_per_class)].set(class_indices[:num_val_samples_per_class])
            train_indices = train_indices.at[current_train_samples:(current_train_samples + num_train_samples_per_class)].set(class_indices[num_val_samples_per_class:])
    ...
    def train(self, train_dataloader : DataLoader, test_dataloader : DataLoader, val_dataloader : DataLoader,
        ...
        # JIT-compiled training step working with state and parameters
        @jax.jit
        def train_step_jit(net_state, opt_state, images, labels, indices):
            def loss_fn(params):
                # Reconstruct network from graph definition and parameters
                net_temp = nnx.merge(nnx.graphdef(self.net), params)
                full_predictions = net_temp(images)
                # Use static indices array instead of dynamic slicing
                predictions = full_predictions[:, indices]
            ...
            # Compute accuracy
            net_temp = nnx.merge(nnx.graphdef(self.net), new_net_state)
            full_predictions = net_temp(images)
            predictions = full_predictions[:, indices]
            labels_int = jnp.argmax(labels, axis=1)
            accuracy = jnp.mean(predictions.argmax(axis=1) == labels_int)
            
            return new_net_state, new_opt_state, loss_val, accuracy

        # pre-compile by running once with dummy data
        dummy_image = jnp.ones((1, 32, 32, 3))
        dummy_label = jnp.ones((1, self.num_classes))
        
        # Precompute class indices for all possible number of classes (5, 10, 15, ..., 100)
        # This is key to avoiding recompilation
        self.possible_indices = {}
        for num_classes in range(5, self.num_classes + 1, 5):
            self.possible_indices[num_classes] = jnp.array(range(num_classes))
        
        # Use precomputed indices for current number of classes
        dummy_indices = self.possible_indices[self.current_num_classes]
        _ = train_step_jit(nnx.state(self.net), self.optim_state, dummy_image, dummy_label, dummy_indices)

        # JIT-compiled CBP training step
        @jax.jit
        def train_step_cbp_jit(net_state, opt_state, images, labels, class_indices):
            def loss_fn(params):
                # Reconstruct network from graph definition and parameters
                net_temp = nnx.merge(nnx.graphdef(self.net), params)
                predictions = net_temp(images)[:, class_indices]
                labels_int = jnp.argmax(labels, axis=1)
                return optax.softmax_cross_entropy_with_integer_labels(predictions, labels_int).mean(), predictions
            
            (loss_val, predictions), grads = jax.value_and_grad(loss_fn, has_aux=True)(net_state)
            updates, new_opt_state = self.optim.update(grads, opt_state)
            new_net_state = optax.apply_updates(net_state, updates)
            ...
            return new_net_state, new_opt_state, loss_val, accuracy, predictions
        ...
        for e in tqdm(range(self.current_epoch, self.num_epochs)):
            ...
            for step_number, sample in enumerate(train_dataloader):
                ...
                # PyTorch DataLoader gives us torch tensors, convert to numpy then JAX
                ...
                # Use JIT training steps with proper state handling
                net_state = nnx.state(self.net)
                # Use precomputed indices array instead of generating it each time
                indices = self.possible_indices[self.current_num_classes]
                
                if self.use_cbp:
                    new_net_state, self.optim_state, current_loss, current_accuracy, predictions = train_step_cbp_jit(
                        net_state, self.optim_state, image, label, indices
                    )
                    # Update network with new state
                    nnx.update(self.net, new_net_state)
                    
                    # CBP feature processing
                    current_features = []
                    _ = self.net(image, current_features)  # Extract features
                    self.resgnt.gen_and_test(current_features)
                else:
                    new_net_state, self.optim_state, current_loss, current_accuracy = train_step_jit(
                        net_state, self.optim_state, image, label, indices
                    )
                    # Update network with new state
                    nnx.update(self.net, new_net_state)
    ...
    def _save_model_parameters(self):
        ...
        file_name = "index-{0}_epoch-{1}.pkl".format(self.run_index, self.current_epoch)  # Changed to .pkl for JAX
        file_path = os.path.join(model_parameters_dir_path, file_name)

        # Save JAX model state instead of PyTorch state_dict
        model_state = nnx.state(self.net)
        store_object_with_several_attempts(model_state, file_path, storing_format="pickle", num_attempts=10)
