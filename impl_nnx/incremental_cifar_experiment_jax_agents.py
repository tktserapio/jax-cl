...
class IncrementalCIFARExperiment(Experiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        agent_config = {
            'replacement_rate': exp_params.get('replacement_rate', 1e-4),
            'maturity_threshold': exp_params.get('maturity_threshold', 1000),
            'shrink_factor': exp_params.get('shrink_factor', 0.9),
            'perturb_std': exp_params.get('perturb_std', 0.01),
        }

        agent_type = exp_params.get('agent_type', 'vanilla')
        self.agent = AgentFactory.create_agent(agent_type, agent_config)
    ...
    def evaluate_network(self, test_data):
        ...
        for batch_idx, sample in enumerate(test_data):
            ...
            # if batch_idx == 0:
            #     self._print(f"🔍 LABEL DEBUG - Batch {batch_idx}:")
            #     self._print(f"  Original one-hot shape: {test_labels.shape}")
            #     self._print(f"  Converted int labels shape: {test_labels_int.shape}")
            #     self._print(f"  Int labels in batch: {test_labels_int}")
            #     self._print(f"  Unique int labels: {jnp.unique(test_labels_int)}")
            #     self._print(f"  Expected range: 0 to {self.current_num_classes-1}")
                
            #     # Check if labels are in valid range
            #     max_label = jnp.max(test_labels_int)
            #     min_label = jnp.min(test_labels_int)
            #     self._print(f"  Label range: {min_label} to {max_label}")
            #     if max_label >= self.current_num_classes:
            #         self._print(f"  ⚠️  WARNING: Label {max_label} >= num_classes {self.current_num_classes}")

            # all_labels_seen.extend(test_labels_int.tolist())
        ...
        # # 🔍 NEW: Final label distribution check
        # unique_labels = jnp.unique(jnp.array(all_labels_seen))
        # self._print(f"🔍 LABEL DEBUG - All unique labels seen: {unique_labels}")
        # self._print(f"🔍 LABEL DEBUG - Label counts:")
        # for label in unique_labels:
        #     count = sum(1 for x in all_labels_seen if x == label)
        #     self._print(f"  Label {label}: {count} samples")
    ...
    def _compute_hessian(self, tag: str, train_loader: DataLoader, eval_loader: DataLoader, task_id: int):
        ...
        current_state = nnx.state(self.net)
        hvp_train = lambda v: hvp_fn_train(current_state, v)
        hvp_eval = lambda v: hvp_fn_eval(current_state, v)
