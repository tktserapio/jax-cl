...
# define l2_regularization function
def l2_regularization(params_pytree, weight_decay: float):
    if weight_decay == 0.0:
        return 0.0
    l2_sum = sum(jnp.sum(jnp.square(p))
                 for p in jax.tree_util.tree_leaves(params_pytree))
...
class IncrementalCIFARExperiment(Experiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        tx = optax.sgd(learning_rate=self.stepsize, momentum=self.momentum, nesterov=False)

        self.train_state = nnx.Optimizer(self.net, tx)
    ...
    def get_experiment_checkpoint(self):
        ...
        checkpoint = {
            ...
            "optimizer_state": nnx.state(self.train_state), # train state
    ...
    def load_checkpoint_data_and_update_experiment_variables(self, file_path):
        ...
        nnx.update(self.train_state, checkpoint["optimizer_state"])
    ...
    def train(self, train_dataloader : DataLoader, test_dataloader : DataLoader, val_dataloader : DataLoader,
        ...
        def train_step_jit(model, optimizer, images, labels, class_indices):
            def loss_fn(model):
                # forward pass
                logits_full = model(images)
                logits = logits_full[:, class_indices]
                ...
                # L2 regularisation on *all* params
                params = nnx.state(model, nnx.Param)
                l2_term = l2_regularization(params, self.weight_decay)

                return ce_loss + l2_term, logits
        ...
        for e in tqdm(range(self.current_epoch, self.num_epochs)):
            ...
            for step_number, sample in enumerate(train_dataloader):
                ...
                current_loss, predictions = train_step_jit(
                    self.net, self.train_state, image, labels_int, current_classes
                )
    ...
    def set_lr(self):
        ...
        if current_stepsize is not None:
            tx = optax.sgd(learning_rate=current_stepsize, momentum=self.momentum, nesterov=False)
            self.train_state = nnx.Optimizer(self.net, tx)
    ...
    def _compute_hessian(self, tag: str, train_loader: DataLoader, eval_loader: DataLoader, task_id: int):
        ...
        jax.debug.callback(
            ...
            task_id, "cifar_jax_manual_l2", at_init=at_init
        )
