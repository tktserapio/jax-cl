...
class IncrementalCIFARExperiment(Experiment):
    ...
    def set_lr(self):
        ...
        if current_stepsize is not None:
            ...
            # But preserve the optimizer state to avoid losing momentum
            old_optim_state = self.optim_state
            self.optim = optax.sgd(learning_rate=current_stepsize, momentum=self.momentum)
            
            # Try to preserve momentum state if possible
            try:
                self.optim_state = old_optim_state  # Keep existing momentum
            except:
                # If incompatible, reinitialize (shouldn't happen with SGD but safety check)
                self.optim_state = self.optim.init(nnx.state(self.net))
