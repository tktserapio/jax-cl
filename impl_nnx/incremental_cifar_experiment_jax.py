...
class IncrementalCIFARExperiment(Experiment):
    ...
    def set_lr(self):
        ...
        if current_stepsize is not None:
            # CRITICAL: Match PyTorch behavior exactly
            # PyTorch: g['lr'] = current_stepsize (preserves momentum state)
            # JAX: We need to recreate optimizer but preserve momentum
            
            old_optim_state = self.optim_state
            self.optim = optax.sgd(learning_rate=current_stepsize, momentum=self.momentum)
            
            # ALWAYS preserve momentum state - this is key to matching PyTorch behavior
            # The "momentum shock" happens because momentum from old classes meets new classes
            self.optim_state = old_optim_state
