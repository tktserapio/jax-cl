...
class IncrementalCIFARExperiment(Experiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        self.num_workers = access_dict(exp_params, key="num_workers", default=0, val_type=int)  # set to 0 for JAX compatibility
    ...
    def train(self, train_dataloader : DataLoader, test_dataloader : DataLoader, val_dataloader : DataLoader,
        ...
        def train_step_jit(net_state, opt_state, images, labels, class_indices):
            ...
            # Compute loss and gradients with predictions
            ...
            return new_net_state, new_opt_state, loss_val, predictions

        # Pre-compile CBP forward pass if needed
        if self.use_cbp:
            @jax.jit
            def cbp_forward_jit(net_state, images, features_list):
                net_temp = nnx.merge(nnx.graphdef(self.net), net_state)
                return net_temp(images, features_list)
        ...
        for e in tqdm(range(self.current_epoch, self.num_epochs)):
            ...
            for step_number, sample in enumerate(train_dataloader):
                ...
                # Get current state and class indices once
                net_state = nnx.state(self.net)
                class_indices = self.all_classes[:self.current_num_classes]
                labels_int = jnp.argmax(label, axis=1)

                # Single forward pass with efficient handling
                if self.use_cbp:
                    # For CBP, we need features - handle this carefully to avoid double forward pass
                    current_features = []
                    _ = self.net(image, current_features)  # Populate features without gradients
                    
                    # Do the training step that includes CBP feature extraction
                    def loss_fn_cbp(params):
                        net_temp = nnx.merge(nnx.graphdef(self.net), params)
                        predictions = net_temp(image, current_features)[:, class_indices]
                        labels_int = jnp.argmax(label, axis=1)
                        return optax.softmax_cross_entropy_with_integer_labels(predictions, labels_int).mean(), predictions
                    
                    (current_loss, predictions), grads = jax.value_and_grad(loss_fn_cbp, has_aux=True)(net_state)
                    updates, self.optim_state = self.optim.update(grads, self.optim_state)
                    new_net_state = optax.apply_updates(net_state, updates)
                    nnx.update(self.net, new_net_state)
                    
                    # CBP processing with the features from the forward pass
                    self.resgnt.gen_and_test(current_features)
                else:
                    # No CBP - single efficient forward pass
                    new_net_state, self.optim_state, current_loss, predictions = train_step_jit(
    ...
    def set_lr(self):
        ...
        if current_stepsize is not None:
            # FIXED: Properly handle optimizer state when changing learning rate
            # Instead of preserving old state (which causes corruption), 
            # we need to reinitialize the optimizer state for the new LR
            ...
            # Reinitialize optimizer state for the new optimizer structure
            # This is the correct way to handle LR changes in JAX/Optax
            self.optim_state = self.optim.init(nnx.state(self.net))
            
            # Log the behavior for debugging
            if (self.current_epoch % self.class_increase_frequency) == 0:
                self._print(f"\tTask boundary: LR reset to {current_stepsize:.5f}, optimizer state reset")
            else:
                self._print(f"\tMid-task: LR decay to {current_stepsize:.5f}, optimizer state reset")
