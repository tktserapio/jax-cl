...
class IncrementalCIFARExperiment(Experiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        # initialize optimizer - JAX/Optax style with weight decay for Shrink & Perturb
        if self.weight_decay > 0.0:
            # Combine SGD with weight decay for complete Shrink & Perturb implementation
            self.optim = optax.chain(
                optax.sgd(learning_rate=self.stepsize, momentum=self.momentum),
    ...
    def evaluate_network(self, test_data):
        ...
        for batch_idx, sample in enumerate(test_data):
            ...
            test_labels = jnp.array(sample["label"].numpy())
            ...
            # Forward pass - select only current classes
            full_predictions = self.net(images)
            test_predictions = full_predictions[:, self.all_classes[:self.current_num_classes]]

            # Debug for first evaluation batch
            if batch_idx == 0 and self.current_epoch == 0:
                self._print(f"Debug EVAL - Test label range: {jnp.min(test_labels_int)} to {jnp.max(test_labels_int)}")
                self._print(f"Debug EVAL - Full predictions shape: {full_predictions.shape}")
                self._print(f"Debug EVAL - Selected predictions shape: {test_predictions.shape}")
                self._print(f"Debug EVAL - Class indices: {self.all_classes[:self.current_num_classes]}")
                pred_int = test_predictions.argmax(axis=1)
                self._print(f"Debug EVAL - First 10 test labels: {test_labels_int[:10]}")
                self._print(f"Debug EVAL - First 10 predictions: {pred_int[:10]}")
    ...
    def train(self,
          ...
          val_data: CifarDataSet):

        # ── build jitted step once; we’ll rebuild if network is reset ──
        def _build_train_step():
            graph_def = nnx.graphdef(self.net)          # single traversal

            @partial(jax.jit, static_argnames=("class_indices",))
            def _step(params, opt_state, images, labels, *, class_indices):
                # Get full logits from the model
                full_logits = nnx.merge(graph_def, params)(images)
                
                # Select only the relevant class logits
                logits = full_logits[:, class_indices]
                
                # Convert one-hot labels to integers (0, 1, 2, 3, 4)
                tgt = jnp.argmax(labels, axis=1)
                
                # Compute loss - this should be learning to map:
                # logits[:, 0] (class_indices[0]=8) -> when tgt=0
                # logits[:, 1] (class_indices[1]=44) -> when tgt=1, etc.
        ...
        params      = nnx.state(self.net)
        opt_state   = self.optim_state

        # ── partition data for first task ──────────────────────────────
        current_subset = slice(0, self.current_num_classes)
        training_data.select_new_partition(self.all_classes[current_subset])
        test_data.select_new_partition(self.all_classes[current_subset])
        val_data.select_new_partition(self.all_classes[current_subset])
        ...
        for e in tqdm(range(self.current_epoch, self.num_epochs)):
            ...
            for step_number, sample in enumerate(train_dataloader):
                ...
                # Debug: Print label information for the first batch
                if step_number == 0 and self.current_epoch == 0:
                    label_int = jnp.argmax(label, axis=1)
                    self._print(f"Debug - First batch label range: {jnp.min(label_int)} to {jnp.max(label_int)}")
                    self._print(f"Debug - Current classes: {self.all_classes[:self.current_num_classes]}")
                    self._print(f"Debug - Label shape: {label.shape}, Image shape: {image.shape}")

                params, opt_state, loss_val, preds = train_step(
                    params,
                    opt_state,
                    image,
                    label,
                    class_indices=tuple(self.all_classes[:self.current_num_classes])
                )
                
                label_int = jnp.argmax(label, axis=1)
                pred_int = preds.argmax(axis=1)
                acc = jnp.mean(pred_int == label_int)
                self.running_loss     += float(loss_val)
                self.running_accuracy += float(acc)

                if (step_number + 1) % self.running_avg_window == 0:
                    self._print(f"\t\tstep {step_number + 1}")
                    self._store_training_summaries()

                # sync model / optimiser back to object fields --------------
                nnx.update(self.net, params)

            epoch_time = time.perf_counter() - epoch_start
            self._store_test_summaries(test_dataloader, val_dataloader,
                                    epoch_number=e,
            ...
            if (self.current_epoch % self.class_increase_frequency) == 0:
                ...
                # if the net was rebuilt, rebuild the compiled step too
                if self.reset_head or self.reset_network:
                    train_step = _build_train_step()
                    params     = nnx.state(self.net)
    ...
    def set_lr(self):
        ...
        if current_stepsize is not None:
            # Match PyTorch behavior exactly: just update learning rate, preserve momentum state
            # In JAX/Optax, we need to create a new optimizer with the new LR but keep the old state
            if self.weight_decay > 0.0:
                self.optim = optax.chain(
                    optax.sgd(learning_rate=current_stepsize, momentum=self.momentum),
                    optax.add_decayed_weights(self.weight_decay)
                )
            else:
            ...
            if (self.current_epoch % self.class_increase_frequency) == 0:
                self._print(f"\tTask boundary: LR reset to {current_stepsize:.5f} (preserving momentum = MOMENTUM SHOCK)")
            else:
                self._print(f"\tMid-task: LR decay to {current_stepsize:.5f} (preserving momentum)")
    ...
    def inject_noise(self):
        ...
        # Use tree.map for clean, efficient noise injection
        def add_noise_to_param(param, key_counter=[0]):
            if hasattr(param, 'shape') and hasattr(param, 'dtype'):  # JAX array
                # Generate unique subkey for each parameter
                param_key = jax.random.fold_in(noise_key, key_counter[0])
                key_counter[0] += 1
                noise = jax.random.normal(param_key, param.shape) * self.noise_std
                return param + noise
            else:
                return param  # Non-parameter values (scalars, etc.)
        
        # Apply noise to all parameters in the tree
        noisy_state = jax.tree.map(add_noise_to_param, current_state)
