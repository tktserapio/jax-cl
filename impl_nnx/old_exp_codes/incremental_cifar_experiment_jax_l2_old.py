...
def main():
    ...
    tx = optax.chain(
        ...
        optax.sgd(learning_rate=stepsize, momentum=momentum, nesterov=False), 
    )
    ...
    np.random.seed(random_seed)
    num_classes = 100
    all_classes = np.random.permutation(num_classes)  # Random class order for incremental learning
...
def run_incremental_experiment(model, state, data_path, all_classes, current_num_classes, batch_sizes, num_workers, config):
    ...
    training_data.select_new_partition(all_classes[:current_num_classes])
    test_data.select_new_partition(all_classes[:current_num_classes])
    val_data.select_new_partition(all_classes[:current_num_classes])
    ...
    for epoch in range(num_epochs):
        ...
        if task_epoch == 0:    # Task start: reset to initial LR
            current_stepsize = config['stepsize']  # 0.1
        elif task_epoch == 60:  # First decay
            current_stepsize = round(config['stepsize'] * 0.2, 5)  # 0.02
        elif task_epoch == 120: # Second decay  
            current_stepsize = round(config['stepsize'] * 0.04, 5)  # 0.004
        elif task_epoch == 160: # Third decay
            current_stepsize = round(config['stepsize'] * 0.008, 5) # 0.0008
        
        if current_stepsize is not None:
            tx = optax.chain(
                optax.add_decayed_weights(config['weight_decay']),
                optax.sgd(learning_rate=current_stepsize, momentum=config['momentum'], nesterov=False)
            )
            current_state = current_state.replace(tx=tx)
            if verbose:
                print(f"\t\tLearning rate updated to: {current_stepsize:.5f}")
        ...
        current_classes = jnp.array(all_classes[:current_num_classes]) # e.g. [23,45,67,12,89]
        ...
        for step, sample in enumerate(train_loader):
            ...
            if (step + 1) % running_avg_window == 0:
        ...
        if current_epoch % class_increase_frequency == 0:
            if current_num_classes < num_classes_total:
                num_classes_increment = 5
                current_num_classes += num_classes_increment
                training_data.select_new_partition(all_classes[:current_num_classes])
                test_data.select_new_partition(all_classes[:current_num_classes])
                val_data.select_new_partition(all_classes[:current_num_classes])
